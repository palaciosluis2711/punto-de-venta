...
export const PurchaseForm: React.FC<PurchaseFormProps> = ({ onSubmit, onCancel, isProcessing, initialData }) => {
    ...
    const [supplierId, setSupplierId] = useState(() => {
        if (initialData) return initialData.supplierId;
        const saved = localStorage.getItem('app_purchase_form_state');
        if (saved) {
            try { return JSON.parse(saved).supplierId || ''; } catch { }
        }
        return '';
    });
    ...
    // Handle initialData changes if they happen after mount (e.g. edit mode loading)
    useEffect(() => {
        if (initialData) {
    ...
    return (
        <>
            ...
            <ConfirmDialog
                ...
                message="Tienes productos agregados. Si cancelas ahora, la compra no se guardará y perderás los datos ingresados."
